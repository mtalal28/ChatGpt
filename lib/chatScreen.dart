...
class _ChatScreenState extends State<ChatScreen> {
  ...
  late DatabaseReference _chatDatabaseReference;
  final DatabaseReference _databaseReference =
  FirebaseDatabase.instance.reference().child('chat_messages'); // Reference to the Firebase Realtime Database
  ...
  void initState() {
    ...
    _databaseReference.onChildAdded.listen((event) {
      // Handle new child message added to the database
    ...
    // Create the database reference using the unique identifier
    _chatDatabaseReference =
        FirebaseDatabase.instance.reference().child('chat_rooms/$uniqueIdentifier');
  }
                  ...
                  children: <Widget>[
                    ...
                    ListTile(
                      title: const Text(
                        'Real-Time Data',
                        style: TextStyle(
                          color: Colors.white, // Set the item text color to white
                        ),
                      ),
                      onTap: () {
                        Navigator.of(context).push(
                    ...
                    ListTile(
                      ...
                      onTap: () {
                        // Add your sidebar item logic here
                      },
                    ),
                  ],
                ),
  ...
  Future<void> sendMessageFCT({
    ...
    required ChatProvider chatProvider, required DatabaseReference databaseReference,
  }) async {
    if (_isTyping) {
    ...
    try {
      ...
      final chatGptResponse = await chatProvider.sendMessageAndGetAnswers(
        ...
        databaseReference: _chatDatabaseReference, // Use the chat-specific database reference here
      );

      // Debugging: Print the AI response
      print('AI Response: $chatGptResponse');

      // Store the chat message in the specific chat room's Firebase Realtime Database
      try {
        await _chatDatabaseReference.push().set({
          'message': msg,
